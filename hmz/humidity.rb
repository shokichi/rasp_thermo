...
opt.on("--tmep=[value]"){|v| OPT[:temp] = v}
opt.on("--volt=[value]"){|v| OPT[:volt] = v}
opt.parse!(ARGV)
...
if OPT.has_key? :test
  temp = 15.0
  volt = 1.00
  puts volt2humidity_hmz(volt,temp)
elsif OPT.has_key? :temp and OPT.has_key? :volt
  volt2humidity_hmz(OPT[:volt],OPT[:temp])
else
  puts "*** USAGE ***"
  puts "./humidity.rb --temp=[value] --volt=[value]"
end
